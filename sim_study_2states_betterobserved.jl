...
logl = loglikelihood_given_hidden_path( T, T_thin, X_thin, d)

function loglikelihood_given_hidden_path_2( T_S::Array{Float64,1}, T_Y::Array{Float64,1}, Y::Array{Int64,1}, n_states::Int64)
    logl = [ zeros(Float64,n_states) for i in 1:length(T_S) ]
    for i in 1:(length(T_S)-1)
        run_loc = ( T_S[i] .<= T_Y .< T_S[i+1] )
        if ! iszero( run_loc )
            logl[i] = repeat([-Inf],n_states)
            s_loc = Y[run_loc][1]
            if all( Y[run_loc][2:end] .== s_loc )
                logl[i][s_loc]  = 0.0
            end
        end
    end
...
logl_2 = loglikelihood_given_hidden_path_2( T, T_thin, X_thin, d)
...
chains_metro
D₀vec  = [ [chains_metro[1][j][1], chains_metro[2][j][1]] for j in 1:4 ]
...
chains_fearn, tiemps_fearn = stationary_diagnost_fearnhead(4,d,3, 2.0, 1.0, 1.0, D₀vec)
...
tiemp_metro_stat  = @elapsed chain_metro_stat = metr_hast_chain_D_P_mode( 2, X_thin, Δ, 15000, 0.05, 1.0, 2.0, 1.0, 1.0, (D₀metro_stat,P₀))
...
tiemp_rao_stat  = @elapsed chain_rao_stat = raotehHMMGibbs( 2, k, T_thin, T_thin[end], X_thin, [1.0,0.0], 2.0, 1.0, ones(2,1), (D₀rao_stat,P₀))
...
tiemp_fearn_stat  = @elapsed chain_fearn_stat = fearnhead_gibbs( 2, X_thin, T_thin, k, 2.0, 1.0, ones(2,1), (D₀fearn_stat,P₀))
...
f(x) = posterior( x[1], x[2], Δ, n_eq_1, n_eq_2, n_ch_1, n_ch_2, 2.0, 1.0)
...
marg1 = [ hquadrature( i -> f_norm([x,i]), 0.0, 10.0 )[1] for x in collect(0.0:0.01:2.0) ]
...
p1_r_stat_5 = histogram( λ_1_rao_stat_5, bins=100, xlim=(0.5,2), color=:lightblue, normalize=true, grid=false, label="Gibbs sampler 2 histogram", legendfontsize=12, xtickfontsize=9, ytickfontsize=9, dpi=300)
plot!(collect(0.0:0.01:2.0),marg1,linewidth=2.5, color=:red, label="True posterior marginal", legendfontsize=12, xtickfontsize=9, ytickfontsize=9, dpi=300)
#ylabel!("f_{\\lambda_1 \\, | \\, X }")
title!("\\lambda_1 posterior marginal distribution")
savefig("lambda1histraostat5.png")

p1_m_stat_5 = histogram( λ_1_metro_stat_5, bins=100, xlim=(0.5,2), color=:lightblue, normalize=true, grid=false, label="M-H sampler histogram", legendfontsize=12, xtickfontsize=9, ytickfontsize=9, dpi=300)
plot!(collect(0.0:0.01:2.0),marg1,linewidth=2.5, color=:red, label="True posterior marginal", legendfontsize=12, xtickfontsize=9, ytickfontsize=9, dpi=300)
#ylabel!("\$ f_{\\lambda_1 \\, | \\, X } \$")
title!("\\lambda_1 posterior marginal distribution")
...
p1_f_stat_5 = histogram( λ_1_fearn_stat_5, bins=100, xlim=(0.5,2), color=:lightblue, normalize=true, grid=false, label="Gibbs sampler 1 histogram", legendfontsize=12, xtickfontsize=9, ytickfontsize=9, dpi=300)
plot!(collect(0.0:0.01:2.0),marg1,linewidth=2.5, color=:red, label="True posterior marginal", legendfontsize=12, xtickfontsize=9, ytickfontsize=9, dpi=300)
#ylabel!("\$ f_{\\lambda_1 \\, | \\, X } \$")
title!("\\lambda_1 posterior marginal distribution")
...
p1_r_5 = histogram( chains_rao[1][1], bins=100, xlim=(0.5,2), color=:pink, normalize=true, grid=false, label="M-H sampler histogram", legendfontsize=12, xtickfontsize=9, ytickfontsize=9, dpi=300)
plot!(collect(0.0:0.01:2.0),marg1,linewidth=2.5, color=:red, label="True posterior marginal", legendfontsize=12, xtickfontsize=9, ytickfontsize=9, dpi=300)
#ylabel!("\$ f_{\\lambda_1 \\, | \\, X } \$")
title!("\\lambda_1 posterior marginal distribution")

p1_m_5 = histogram( chains_metro[1][1], bins=100, xlim=(0.5,2), color=:pink, normalize=true, grid=false, label="M-H sampler histogram", legendfontsize=12, xtickfontsize=9, ytickfontsize=9, dpi=300)
plot!(collect(0.0:0.01:2.0),marg1,linewidth=2.5, color=:red, label="True posterior marginal", legendfontsize=12, xtickfontsize=9, ytickfontsize=9, dpi=300)
#ylabel!("\$ f_{\\lambda_1 \\, | \\, X } \$")
title!("\\lambda_1 posterior marginal distribution")
...
@load "CHAIN_rao_50.txt"

s_1000 = [ CHAIN_rao_50[4][i][findlast( CHAIN_rao_50[3][i] .<= 1000.5 )] for i in 1:length(CHAIN_rao_50[3]) ]
...
prop_correct_s_estim = [ mean( X[findlast( T .<= j )]  .== [ CHAIN_rao_50[4][i][findlast( CHAIN_rao_50[3][i] .<= j )] for i in 1:length(CHAIN_rao_50[3]) ]) for j in collect( 0.5:1.0:(T_thin[end]-0.5) )]
...
histogram(prop_correct_s_estim,normalize=true)
